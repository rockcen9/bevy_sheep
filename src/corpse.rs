...
impl Plugin for CorpsePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (corpse_system, spawn_corpse_system).in_set(GameSet::Playing),
        )
...
fn corpse_system(mut commands: Commands, time: Res<Time>, mut query: Query<(Entity, &mut Corpse)>) {
...
fn spawn_corpse_system(
    ...
    for event in event.read() {
        commands.spawn((
            Mesh3d(storage.mesh.clone()),
            MeshMaterial3d(storage.material.clone()),
            Transform::from_translation(
                event.position + Vec3::Y * rand::thread_rng().gen_range(0.01..=0.02),
            )
